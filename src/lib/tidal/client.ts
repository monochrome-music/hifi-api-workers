...
function getCacheKey(env: Bindings): string {
  ...
  return credentials ? `${credentials.clientId}:${credentials.refreshToken}` : ''
}
...
async function fetchWithTimeout(input: RequestInfo | URL, init: RequestInit, timeoutMs: number): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new ApiError(429, 'Upstream timeout')
    }
...
async function refreshAccessToken(env: Bindings): Promise<string> {
  ...
  const credentials = getTidalCredentials(env)
  if (!credentials) {
    throw new ApiError(500, 'No Tidal credentials available; configure CLIENT_ID, CLIENT_SECRET, and REFRESH_TOKEN')
  }
    ...
    tokenCache.refreshPromise = (async () => {
      ...
      const response = await fetchWithTimeout(
        'https://auth.tidal.com/v1/oauth2/token',
        {
          method: 'POST',
          headers: {
            authorization: basicAuthorization(credentials.clientId, credentials.clientSecret),
      ...
      if (!response.ok) {
        ...
        throw new ApiError(401, `Token refresh failed: ${response.status} ${response.statusText} ${errorText}`)
      }
...
export async function getAccessToken(env: Bindings, forceRefresh = false): Promise<string> {
  resetCacheIfCredentialsChanged(env)
...
export async function tidalJsonRequest(options: JsonRequestOptions): Promise<{ data: any; token: string }> {
...
export async function tidalProxyRequest(options: RawRequestOptions): Promise<Response> {
  let token = options.token ?? (await getAccessToken(options.env))
  const target = buildUrl(options.url, options.params)
